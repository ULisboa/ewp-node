...
class EwpApiInterInstitutionalAgreementsV7ControllerTest
  ...
  @Autowired private IiaHashServiceV7 iiaHashService;
  ...
  public void
    ...
    for (int index = 0; index < iiaIds.size(); index++) {
      ...
      studentStudiesMobilitySpec.setSendingHeiId(UUID.randomUUID().toString());
      studentStudiesMobilitySpec.setReceivingHeiId(UUID.randomUUID().toString());
      studentStudiesMobilitySpec.setReceivingOunitId(ounitIds.get(index));
      studentStudiesMobilitySpec.setReceivingFirstAcademicYearId("2021/2022");
      studentStudiesMobilitySpec.setReceivingLastAcademicYearId("2021/2022");
      studentStudiesMobilitySpec.setTotalMonthsPerYear(BigDecimal.TEN);
      ...
      studentStudiesMobilitySpec.getReceivingContact().add(receivingContact);
      ...
      doReturn(
              Optional.of(
                  EwpInterInstitutionalAgreementMapping.create(
                      heiId, ounitIds.get(index), iiaIds.get(index), UUID.randomUUID().toString())))
    ...
    doReturn(Optional.ofNullable(mockProvider1))
        .when(hostPluginManager)
        .getSingleProvider(
            heiId, ounitIds.get(0), InterInstitutionalAgreementsV7HostProvider.class);
    doReturn(Optional.ofNullable(mockProvider2))
        .when(hostPluginManager)
        .getSingleProvider(
            heiId, ounitIds.get(1), InterInstitutionalAgreementsV7HostProvider.class);
    doReturn(Optional.ofNullable(mockProvider2))
        .when(hostPluginManager)
        .getSingleProvider(
            heiId, ounitIds.get(2), InterInstitutionalAgreementsV7HostProvider.class);
    ...
    for (Iia iia : response.getIia()) {
      assertThat(iia.getIiaHash())
          .isEqualTo(this.iiaHashService.calculateIiaHashes(List.of(iia)).get(0).getHash());
    }
  }
  ...
  public void
    ...
    MockInterInstitutionalAgreementsV7HostProvider mockProvider1 =
        new MockInterInstitutionalAgreementsV7HostProvider(0, 3);
    MockInterInstitutionalAgreementsV7HostProvider mockProvider2 =
        new MockInterInstitutionalAgreementsV7HostProvider(0, 3);
  ...
  @Test
  public void testStatsRetrieval_InvalidHeiId_ErrorReturned() throws Exception {
  ...
  public void testStatsRetrieval_ValidRequesterHeiIdAndTwoHostProviders_StatsMergedReturned()
    ...
    MockInterInstitutionalAgreementsV7HostProvider mockProvider1 =
        new MockInterInstitutionalAgreementsV7HostProvider(3, 3);
    ...
    String responseXml =
        executeRequest(
            ...
            .getResponse()
            .getContentAsString();
    IiasStatsResponseV7 response = XmlUtils.unmarshall(responseXml, IiasStatsResponseV7.class);

    assertThat(response).isNotNull();
    assertThat(response.getIiaFetchable()).isEqualTo(BigInteger.valueOf(12L));
    assertThat(response.getIiaLocalUnapprovedPartnerApproved()).isEqualTo(BigInteger.valueOf(14L));
    assertThat(response.getIiaLocalApprovedPartnerUnapproved()).isEqualTo(BigInteger.valueOf(16L));
    assertThat(response.getIiaBothApproved()).isEqualTo(BigInteger.valueOf(18L));
  }
}
